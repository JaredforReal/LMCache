...
def analyze_performance(results):
    ...
    for scenario, df in results.items():
        ...
        metrics["throughput_rps"] = (
            metrics["total_requests"] / metrics["total_duration"]
        )
        metrics["input_tokens_per_sec"] = (
            metrics["total_input_tokens"] / metrics["total_duration"]
        )
        metrics["output_tokens_per_sec"] = (
            metrics["total_output_tokens"] / metrics["total_duration"]
        )
...
def compare_scenarios(summary):
    ...
    for scenario, metrics in summary.items():
        ...
        ttft_percent = (
            (baseline["mean_ttft"] - metrics["mean_ttft"]) / baseline["mean_ttft"] * 100
        )
        throughput_percent = (
...
def plot_results(results, output_dir="plots"):
    ...
    if len(results) > 1:
            ...
            metrics_data.append(
                {
                    ...
                    "Throughput": len(df)
                    / (df["finish_time"].max() - df["launch_time"].min()),
                }
            )
        ...
        plt.tight_layout()
        plt.savefig(
            f"{output_dir}/metrics_comparison.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="plots", help="Directory for output plots"
    )
    ...
    if "," in args.pattern:
        ...
        files_to_load = [
            str(Path(summary_dir) / f.strip()) for f in args.pattern.split(",")
        ]
    else:
        # Handle glob pattern
