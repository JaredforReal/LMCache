...
class TraceReplayer:
    ...
    def load_trace(self, trace_file: str = "trace.jsonl") -> List[TraceRequest]:
        ...
        try:
            ...
            if requests:
                ...
                requests = [
                    req for req in requests if req.timestamp <= self.max_duration
                ]
    ...
    async def replay_trace(self, requests: List[TraceRequest], summary_dir: str = "."):
        ...
        for request in requests:
            ...
            logger.info(
                f"Launched request {request.chat_id} at "
                f"{request.timestamp:.2f}s "
...
class TraceDataGenerator:
    ...
    def generate_synthetic_conversation_data(
        ...
        conversation_templates = [
            ...
            (
                "I'm building a web application and need advice on architecture.",
                "",
                180,
            ),
        ...
        long_document_templates = [
            (
                "Based on this research paper, what are the main conclusions? Paper: ",
                (
                    "Abstract: "
                    + "Deep learning has revolutionized artificial intelligence "
                    "by enabling machines to learn complex patterns from data. "
                )
                * 100,
                200,
            ),
            (
                "Analyze this legal document and identify key clauses: ",
                "CONTRACT AGREEMENT: "
                + "The parties agree to the following terms and conditions. " * 80,
                150,
            ),
            (
                "Review this medical case study: ",
                "Patient History: "
                + "A 45-year-old patient presented with symptoms including. " * 60,
                180,
            ),
        ]
        ...
        logger.info(
            ...
            f"Average rate: {len(requests_data) / duration:.2f} req/s"
        )

    def convert_sharegpt_to_trace(
        self,
        sharegpt_file: str,
        ...
        timestamps = sorted(
            [
                ...
                for _ in range(min(max_requests, len(sharegpt_data)))
            ]
        )

        for i, (conversation, timestamp) in enumerate(
            zip(sharegpt_data[:max_requests], timestamps)
        ):
            try:
    ...
    def generate_caching_focused_trace(
        self,
        num_requests: int = 200,
        duration: float = 120.0,
        cache_hit_ratio: float = 0.6,
        output_file: str = "cache_trace.jsonl",
    ) -> None:
        ...
        for i in range(20):  # 20 different base documents
            doc_content = f"Document {i}: " + " ".join(
                [f"content_word_{j}" for j in range(1000)]
            )
        ...
        logger.info(
            f"Generated {len(requests_data)} cache-focused requests in {output_file}"
        )
        actual_hits = sum(1 for r in requests_data if "hit" in str(r["chat_id"]))
        logger.info(
            f"Target cache hit ratio: {cache_hit_ratio:.1%}, "
            f"Actual: {actual_hits / len(requests_data):.1%}"
        )
